...
export async function exampleBasicUsage() {
  ...
  console.log(
    'First 3 cantons:',
    cantons.slice(0, 3).map((c) => `${c.cantonAbbreviation}: ${c.cantonLongName}`),
  )
  ...
  console.log(
    ...
    municipalities.slice(0, 3).map((m) => `${m.municipalityLongName} (${m.cantonAbbreviation})`),
  )
}
...
export async function exampleLookups() {
  ...
  if (municipality) {
    console.log(
      `GDN ID 0100230: ${municipality.municipalityLongName} in ${municipality.cantonAbbreviation}`,
    )
  }
  ...
  console.log(
    'First 5:',
    zurichMunicipalities.slice(0, 5).map((m) => m.municipalityLongName),
  )
}
...
export async function exampleSearch() {
  ...
  console.log(
    'Cantons:',
    searchResult1.cantons.map((c) => c.cantonLongName),
  )
  console.log(
    'Municipalities:',
    searchResult1.municipalities.map((m) => m.municipalityLongName),
  )
...
export async function exampleIntegration() {
  ...
  const municipalitiesWithData = municipalities.filter(
    (m) => m.cantonAbbreviation === 'ZH' || m.cantonAbbreviation === 'AG',
  )

  console.log(`Municipalities with financial data: ${municipalitiesWithData.length}`)
  console.log(
    'Examples:',
    municipalitiesWithData.slice(0, 5).map((m) => `${m.municipalityLongName} (${m.gdnId})`),
  )
