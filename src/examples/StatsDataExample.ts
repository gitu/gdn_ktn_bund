...
export async function exampleBasicUsage() {
  ...
  try {
    ...
    console.log('Available statistics:', availableStats.map(s => s.id));
...
export async function exampleStatisticalAnalysis() {
  ...
  try {
    ...
    console.log(`  Largest: ${cantonSummary.max.entity} (${cantonSummary.max.value.toLocaleString()})`);
    console.log(`  Smallest: ${cantonSummary.min.entity} (${cantonSummary.min.value.toLocaleString()})`);
    console.log(`  Standard deviation: ${Math.round(cantonSummary.standardDeviation).toLocaleString()}`);
    ...
    console.log(`  Largest: ${municipalitySummary.max.entity} (${municipalitySummary.max.value.toLocaleString()})`);
    console.log(`  Smallest: ${municipalitySummary.min.entity} (${municipalitySummary.min.value.toLocaleString()})`);
  ...
  try {
    ...
    if (availableYears.ktn.length >= 2) {
      ...
      comparison.changes
        ...
        .slice(0, 5)
        .forEach(change => {
          console.log(`  ${change.geoName}: +${change.percentageChange.toFixed(2)}% (+${change.absoluteChange.toLocaleString()})`);
        });
...
export async function exampleErrorHandling() {
  ...
  try {
    ...
    console.log('Expected error for non-existent statistic:', error instanceof Error ? error.message : String(error));
  }
  ...
  try {
    ...
    console.log('Expected error for non-existent year:', error instanceof Error ? error.message : String(error));
  }
