...
export class StatsDataLoader {
  ...
  async loadStatsCatalog(): Promise<StatsCatalog> {
    ...
    try {
      ...
      throw new Error(`Failed to load stats catalog: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
   ...
   */
  private applyFilters(records: ProcessedStatsRecord[], filters?: StatsDataFilters): ProcessedStatsRecord[] {
    ...
    return records.filter(record => {
      ...
      if (filters.geoNamePattern && !record.geoName.toLowerCase().includes(filters.geoNamePattern.toLowerCase())) {
        return false;
      }
   ...
   */
  async loadGdnData(statsId: string, year: number, filters?: StatsDataFilters): Promise<StatsDataResult> {
    const entry = await this.getStatsEntry(statsId);
    if (!entry) {
      throw new Error(`Statistics entry not found: ${statsId}`);
    }
   ...
   */
  async getCantonData(statsId: string, year: number, cantonId: string): Promise<ProcessedStatsRecord | null> {
   ...
   */
  async getMunicipalityData(statsId: string, year: number, municipalityId: string): Promise<ProcessedStatsRecord | null> {
   ...
   */
  async searchMunicipalities(statsId: string, year: number, namePattern: string): Promise<ProcessedStatsRecord[]> {
   ...
   */
  async getTopEntities(statsId: string, year: number, dataType: 'ktn' | 'gdn', limit: number = 10): Promise<ProcessedStatsRecord[]> {
    const result = dataType === 'ktn'
   ...
   */
  async compareYears(statsId: string, year1: number, year2: number, dataType: 'ktn' | 'gdn'): Promise<{
   ...
   */
  async getStatsSummary(statsId: string, year: number, dataType: 'ktn' | 'gdn'): Promise<{
    total: number;
    average: number;
    median: number;
 ...
 */
export async function loadStatsDataLoader(config?: Partial<StatsDataConfig>): Promise<StatsDataLoader> {
 ...
 */
export async function getCantonPopulation(cantonId: string, year: number = 2023): Promise<number | null> {
  const loader = await loadStatsDataLoader();
 ...
 */
export async function getMunicipalityPopulation(municipalityId: string, year: number = 2023): Promise<number | null> {
  const loader = await loadStatsDataLoader();
