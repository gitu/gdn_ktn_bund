...
describe('ScalingOptimization', () => {
  const mockAvailableStats: StatsAvailabilityInfo[] = [
    {
      ...
      type: 'scaling',
      description: { de: 'Einwohnerzahl', en: 'Population count', fr: "Nombre d'habitants", it: 'Numero di abitanti' },
    ...
    },
    {
      ...
      type: 'scaling',
      description: { de: 'Anzahl Arbeitsplätze', en: 'Number of jobs', fr: "Nombre d'emplois", it: 'Numero di posti di lavoro' },
      availableKtnYears: [2023],
      availableGdnYears: [2023],
      source: 'Test Source',
      lastUpdate: '2024-01-01',
    },
    {
      ...
      type: 'scaling',
      description: { de: 'Gesamtfläche', en: 'Total area', fr: 'Surface totale', it: 'Superficie totale' },
      availableKtnYears: [2023],
      availableGdnYears: [2023],
      source: 'Test Source',
      lastUpdate: '2024-01-01',
  ...
  const createOptimizationTargets = (data: Array<{entity: string, target: number, factors: {[key: string]: number}}>): OptimizationTarget[] => {
  ...
  describe('Linear regression optimization', () => {
    ...
    it('should handle perfect linear relationship', () => {
      ...
      const result = ScalingOptimization.optimizeScalingFormula(targets, mockAvailableStats.slice(0, 1)) // Only use pop
  ...
  describe('Options handling', () => {
    ...
    it('should handle intercept option', () => {
      ...
      const resultWithIntercept = ScalingOptimization.optimizeScalingFormula(targets, mockAvailableStats, {
        includeIntercept: true
      })

      const resultWithoutIntercept = ScalingOptimization.optimizeScalingFormula(targets, mockAvailableStats, {
        includeIntercept: false
      })

      if (resultWithIntercept.isValid && resultWithoutIntercept.isValid) {
        // With intercept should handle the constant offset better
        expect(resultWithIntercept.rSquared).toBeGreaterThanOrEqual(resultWithoutIntercept.rSquared!)
      }
    })
  })
  ...
  describe('Formula generation', () => {
    ...
    it('should omit very small coefficients', () => {
      ...
      const targets = createOptimizationTargets([
        { entity: 'entity1', target: 100, factors: { pop: 100, workplaces: 1000000, total_area: 100000 } },
        { entity: 'entity2', target: 200, factors: { pop: 200, workplaces: 2000000, total_area: 200000 } },
        { entity: 'entity3', target: 300, factors: { pop: 300, workplaces: 3000000, total_area: 300000 } }
      ])
